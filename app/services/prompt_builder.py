...
def prompt_user_builder(data: Questionnaire) -> str:
    prompt = f"""
        Create a fairy tale to read aloud according to the parameters limited by XML-tags:
            ...
            5. Desired tale duration: {data.story_duration_minutes} minutes  
               (base this on an average read-aloud speed of 120 words per minute)
            6. Child's interests: {", ".join(data.subcategories)}
